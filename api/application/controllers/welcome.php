...
class Welcome extends CI_Controller{
	...
	{
        $data['status']="true";
		$Products=$this->admin_model->getData('salo_products',array('id' => $proid),'AND');
		$productArray=array();
		$lr=$Products[0];
			$id=$lr['id'];
			$title=$lr['title'];
	...
	public function registerUser()
	{
	    ...
	    if(($name!="") && ($email!="") && ($contact!="") && ($password!=""))
	    {
			  ...
			  else
			  {
				  ...
				  $getUserContact=$this->admin_model->getData('salo_users',array('contact' => $contact),null);
				  ...
				  else if(count($getUserContact)!=0)
				  {
				  
					$data['status'] = "False";
					$data['type'] = '400';
					$data['error'] = 'Contact Number already registered';
					$data['msg']    = "Done";
				  }
				  else
				  {
				   ...
				   $referral_point=$admindt[0]['referral_point'];
					...
					$data['status'] = "True";
					$data['data']=array('id' => $up,'referral_code' => $referral_code,'referral_point' => $referral_point,'user_type' => '1','name' => $name,'email' => $email,'password' => $password,'contact' => $contact,'first_login_status' => $first_login_status,'wallet_amount' => '0.00');
					$data['type'] = '200';
					$data['success'] = 'true';
					$data['msg']    = "Done";
	...
	public function login()
	{
		...
		if(($email!="") && ($pwd!=""))
	    {
            ...
            $getUserContact=$this->admin_model->getData('salo_users',array('contact' => $email,'password' => $pwd,'status' => 1),'AND');
            ...
            if(count($getUser)!=0)
            {
				   ...
				   $arr=array(array('id' => $uid,'user_type' => $user_type,'referral_code' => $referral_code,'referral_point' => $referral_point,'name' => $uname,'email' => $uemail,'password' => $password,'contact' => $ucontact,'first_login_status' => $first_login_status,'wallet_amount' => $wallet_amount));
            ...
            {
                 $uid=$getUserContact[0]['id'];
				   $user_type=$getUserContact[0]['user_type'];
				   $referral_code=$getUserContact[0]['referral_code'];
				   $wallet_amount=$getUserContact[0]['wallet_amount'];
				   $uname=$getUserContact[0]['name'];
				   $uemail=$getUserContact[0]['email'];
				   $ucontact=$getUserContact[0]['contact'];
				   $password=$getUserContact[0]['password'];
				   $first_login_status=$getUserContact[0]['first_login_status'];
	...
	public function emptyCart($uid,$device_id)
	{
	    if(($uid==0) || ($uid==""))
		{
		...
		$this->admin_model->deleteData('salo_temp_cart',$whereArr);
	...
	public function checkout()
	{
		  ...
		  if($order_id=="")
		  {
			  ...
			  //$newid = $idd + 1 ;
			  $orderid = 'DN'.date('Y').date('m').date('d').'-'.$lastnum ;
		  }
		  else
		  {
		      $orderid=$order_id;
	...
	public function cashonDelivery()
	{
		...
		$this->db->update('salo_temp_cart', array('wallet_amount' => $used_wallet_amount,'final_amount' => $final_amount,'status' => 1) , array('order_id' => $orderId)); 
					 
		$this->db->update('salo_cart_shipping_details', array('wallet_amount' => $used_wallet_amount,'final_amount' => $final_amount,'payment_mode' => 'Cash On Delivery (COD)') , array('order_id' => $orderId)); 
  ...
  <td align="center" style="padding-top:0px"><table align="center" cellpadding="0" cellspacing="0" class="m_2299037813007463152container" style="width:100%">
      ...
      <td align="center" style="text-align:center;vertical-align:top;font-size:0;padding:10px">
        ...
        <div style="width:200px;display:inline-block;vertical-align:top;text-align:center;font-size:0" align="center">
          <table width="180" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-bottom:10px">
            ...
            $fromAdmin="no-reply@dadanani.com";
	...
	public function payonlineSuccess()
	{
		...
		if(count($prot)!=0)
		{
		            ...
		            $this->db->update('salo_temp_cart', array('wallet_amount' => $used_wallet_amount,'final_amount' => $final_amount,'status' => 1,'payment_status' => 1) , array('order_id' => $orderId)); 
					 
					 $this->db->update('salo_cart_shipping_details', array('wallet_amount' => $used_wallet_amount,'final_amount' => $final_amount,'payment_mode' => 'Online Payment','payment_status' => 1) , array('order_id' => $orderId)); 
...
<tr>
  <td align="center" style="padding-top:0px"><table align="center" cellpadding="0" cellspacing="0" class="m_2299037813007463152container" style="width:100%">
	    ...
	    if(($name!="") && ($email!="") && ($contact!="") && ($password!=""))
	    {
			...
			else
			{
			  ...
			  if(count($getUser)==0)
			  {
				...
				$data['status'] = "True";
				$data['data']=array('id' => $up,'referral_code' => $referral_code,'referral_point' => $referral_point,'user_type' => '2','name' => $name,'email' => $email,'password' => $password,'contact' => $contact,'first_login_status' => $first_login_status,'wallet_amount' => '0.00');
				$data['type'] = '200';
				$data['success'] = 'true';
				$data['msg']    = "Done";
	...
	public function offerProducts($mobileDeviceId,$loggedInUserId,$offType,$page=1)
	{
		   $data['status']="True";
		   ...
		   else
		   {
			...
			switch($offType)
			{
				case "combo_pack":
				    ...
				    $productCount=$this->admin_model->getBasic("SELECT id FROM salo_combo_packs where status='1' and offer_type='1'");
		            $total_records=count($productCount);
				    $Products = $this->admin_model->getwithLimitOrderBy('salo_combo_packs',array('status' => '1','offer_type' => '1'),$data['fetchLimit'],$limit,'id','DESC');
				...
				case "buy_get_free":
                    ...
                    $productCount=$this->admin_model->getBasic("SELECT id FROM salo_combo_packs where status='1' and offer_type='2'");
		            $total_records=count($productCount);
				    $Products = $this->admin_model->getwithLimitOrderBy('salo_combo_packs',array('status' => '1','offer_type' => '2'),$data['fetchLimit'],$limit,'id','DESC');
				...
				default:
				  ...
				  $productCount=$this->admin_model->getBasic("SELECT id FROM salo_products WHERE status='1' AND (id IN (SELECT product_id FROM  salo_product_attribute WHERE `discount` <= '$matchVal')) ORDER BY id DESC");
		          $total_records=count($productCount);
				 
				  $endLimit=$data['fetchLimit'];
				  $Products = $this->admin_model->getBasic("SELECT * FROM salo_products WHERE status='1' AND (id IN (SELECT product_id FROM  salo_product_attribute WHERE `discount` <= '$matchVal')) ORDER BY id DESC LIMIT $limit,$endLimit");
		    ...
		    if(($offType!='combo_pack') && ($offType!='buy_get_free'))
			  {
    			...
    			$image=$lr['image'];
    			$stock_status=$lr['stock_status'];
    			$description=strip_tags($lr['description']);
    			$price = $lr['price'];
...
public function getsingleOfferProduct($mobileDeviceId,$loggedInUserId,$proid)
	{
        $data['status']="true";
		$Products=$this->admin_model->getData('salo_combo_packs',array('id' => $proid),'AND');
		$productArray=array();
		$lr=$Products[0];
	    $id=$lr['id'];
		$title=$lr['title'];
	...
	{
	    $getData=$this->admin_model->getData('salo_contents',array('menu' => 'terms-conditions','status' => 1),'AND');
